...
class Graphs(commands.Cog):
    ...
    async def prufer(self, ctx, *, code):
        ...
        if tree is not None:
            nx.draw(tree, with_labels=True, font_weight='bold', font_size=16, width=3, node_color='#e530e8',
                    node_size=800)
            plt.savefig("tree.png")
            plt.clf()
            file = discord.File("tree.png")
            embed = discord.Embed(title="Drzewo z kodu Prüfera:",
                                  description=f"```{code}```",
            ...
            await ctx.send(embed=discord.Embed(title="Niepoprawny Kod Prüfera!",
    ...
    async def rysuj(self, ctx, *, args):
        ...
        if len(args) < 3:
            await ctx.send(embed=discord.Embed(title="Musisz podać 2 parametry",
        ...
        if tree is not None:
            ...
            embed = discord.Embed(title="Graf nieskierowany:",
                                  description=f"",
                                  color=discord.Color.dark_purple())
            ...
            await ctx.send(embed=discord.Embed(title="Błędny zapis listy wierzchołków lub/i krawędzi",
                                               description="Aby uzyskać pomoc wpisz `^help graf`",
                                               color=discord.Color.dark_purple()))
