    ...
    async def summoner_info(self, ctx, name):
        ...
        for x in range(0,len(dataRanks)):
            ...
            if str(dataHelp["queueType"]) == "RANKED_SOLO_5x5":
                rangaPlayer+="Ranga Solo/Duo"+" : "+str(dataHelp["tier"])+" "+str(dataHelp["rank"])+"\n"
            if str(dataHelp["queueType"]) == "RANKED_FLEX_SR":
                rangaPlayer+="Ranga Flex"+" : "+str(dataHelp["tier"])+" "+str(dataHelp["rank"])+"\n"

        urlChampionMastery = "https://eun1.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/"
        dataChampionMastery = requests.get(f'{urlChampionMastery}{data["id"]}',headers=headers)
        ...
        for x in range(0,3):
            ...
            championMastery += str(x+1)+ " : " + nameChampion[str(dataHelp["championId"])]
            championMastery += "\nLVL Mastery : "+ str(dataHelp["championLevel"]) + "\nPunkty Masteri : " + str(dataHelp["championPoints"]) + "\n\n"
        ...
        embed.add_field(name='Ranga : ',
                        value=f'{rangaPlayer}',
                        inline=False)
        embed.add_field(name='Champion top 3 Mastery : ',
